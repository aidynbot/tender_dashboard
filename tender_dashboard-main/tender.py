...
def generate_sample_data(n=350):
    ...
    df["Лат"] = df["Регион"].map(lambda r: region_coords.get(r,(55.75,37.62))[0]) + np.random.uniform(-0.5,0.5,n)
    df["Лон"] = df["Регион"].map(lambda r: region_coords.get(r,(55.75,37.62))[1]) + np.random.uniform(-0.5,0.5,n)
    return df
...
if "Лат" in df.columns:
    mf = st.radio("Карта",["Все","Выигран","Проигран","В процессе"],horizontal=True)
    mdf = df if mf=="Все" else df[df["Статус"]==mf]
    mdf = mdf.copy(); mdf["Размер"]=np.sqrt(mdf["Сумма тендера"]/mdf["Сумма тендера"].max())*30+4
    mdf["Сумма_conv"]=mdf["Сумма тендера"]*display_rate/1e6
    fig_map = px.scatter_geo(mdf,lat="Лат",lon="Лон",color="Статус",size="Размер",
                              hover_name="Заказчик",
                              hover_data={"Регион":True,"Сумма_conv":":.1f","Статус":True,"Лат":False,"Лон":False,"Размер":False},
                              color_discrete_map=STATUS_COLORS,projection="natural earth",
                              labels={"Сумма_conv":f"Сумма (млн {currency_symbol})"})
    fig_map.update_geos(scope="europe",center={"lat":58,"lon":60},projection_scale=3.5,
                         bgcolor="#0d1117",landcolor="#1c2333",oceancolor="#0d1117",showocean=True,
                         lakecolor="#161b22",showcountries=True,countrycolor="#30363d")
    apply_layout(fig_map,title="Карта тендеров",height=460)
